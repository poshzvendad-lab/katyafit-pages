...
const PAY_ENDPOINT = 'https://sxktnpupxcslzjrhisyw.supabase.co/functions/v1/pay-create';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const submitLabel = isTrial ? 'Получить доступ' : 'Перейти к оплате';
    ...
    // ─── Опечатка в адресе почты ──────────────────────────────────────────
    // Живой случай 01.09.2026: женщина ввела «...@mail.ry» вместо «mail.ru».
    // В тот раз она оплату не довела и через неделю вернулась. Заплати она
    // сразу — и доступ, и письмо ушли бы на несуществующий адрес, а деньги
    // остались бы у нас.
    //
    // Подсказываем, но НЕ блокируем и поле ошибкой не красим: адрес бывает и
    // правда необычным, а перегородить оплату страшнее опечатки.
    const DOMAIN_FIX = {
        'mail.ry': 'mail.ru', 'mail.ri': 'mail.ru', 'mail.tu': 'mail.ru',
        'mail.rru': 'mail.ru', 'mail.u': 'mail.ru', 'mai.ru': 'mail.ru',
        'mial.ru': 'mail.ru', 'mali.ru': 'mail.ru', 'maill.ru': 'mail.ru',
        'nail.ru': 'mail.ru', 'mail.com': 'mail.ru',
        'yandex.ry': 'yandex.ru', 'yandex.ri': 'yandex.ru',
        'yandx.ru': 'yandex.ru', 'yandes.ru': 'yandex.ru',
        'yadex.ru': 'yandex.ru', 'yandex.com': 'yandex.ru',
        'ynadex.ru': 'yandex.ru',
        'gmail.con': 'gmail.com', 'gmail.co': 'gmail.com',
        'gmail.cm': 'gmail.com', 'gmail.ocm': 'gmail.com',
        'gmial.com': 'gmail.com', 'gmai.com': 'gmail.com',
        'gmaill.com': 'gmail.com', 'gmail.ru': 'gmail.com',
        'bk.ry': 'bk.ru', 'list.ry': 'list.ru', 'inbox.ry': 'inbox.ru',
        'rambler.ry': 'rambler.ru', 'internet.ry': 'internet.ru',
    ...
    function suggestEmail(value) {
        ...
        if (at < 1) return null;
        const fixed = DOMAIN_FIX[v.slice(at + 1)];
        return fixed ? v.slice(0, at) + '@' + fixed : null;
    }

    function hideEmailSuggestion() {
        if (!emailSuggest) return;
        emailSuggest.classList.remove('is-shown');
        emailSuggest.textContent = '';
    }

    function showEmailSuggestion() {
        if (!emailSuggest || !emailInput) return;
        const fix = suggestEmail(emailInput.value);
        if (!fix) { hideEmailSuggestion(); return; }

        // Собираем через DOM, а не строкой: адрес пришёл от человека,
        // в разметку его подставлять нельзя.
        emailSuggest.textContent = 'Возможно, вы имели в виду ';
        const btn = document.createElement('button');
        btn.type = 'button';
        btn.className = 'form-suggest__fix';
        btn.textContent = fix;
        ...
        emailSuggest.appendChild(btn);
        emailSuggest.appendChild(document.createTextNode('?'));
        emailSuggest.classList.add('is-shown');
    }

    if (emailInput) {
        emailInput.addEventListener('blur', showEmailSuggestion);
        // Пока человек правит адрес, старая подсказка только мешает.
        emailInput.addEventListener('input', hideEmailSuggestion);
    }
    ...
    function validateForm() {
        ...
        // Адрес формально верный, но домен похож на опечатку — показываем
        // подсказку и на попытке отправить, а не только при потере фокуса.
        if (!errors.email) showEmailSuggestion();
    ...
    // ─── 💳 Оплата через ЮKassa ───────────────────────────────────────────
    // Платные тарифы: просим сервер создать платёж и уходим на страницу
    // оплаты. Сумму сервер берёт из базы по коду тарифа — здесь её нет
    // специально, иначе цену можно было бы поправить в консоли браузера.
    //
    // Дальше: человек платит → ЮKassa зовёт pay-webhook → тот сверяет платёж
    // напрямую в ЮKassa, заводит аккаунт по этой почте и открывает доступ.
    //
    // Тест-драйв бесплатный: платежа нет, сервер сам заводит аккаунт и открывает
    // дни 1–3, а Кате шлёт уведомление постфактум.
    async function startCheckout(email) {
        const response = await fetch(PAY_ENDPOINT, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ plan: selectedPlan.id, email: email }),
        });
        ...
        // Тест-драйв: платежа нет. opened — сервер уже открыл дни 1–3;
        // если нет, доступ доделает Катя руками.
        if (data.trial) return { trial: true, opened: !!data.opened };
    ...
    if (checkoutForm) {
        checkoutForm.addEventListener('submit', async (e) => {
            ...
            if (submitBtn) {
                ...
                submitBtn.textContent = isTrial ? 'Отправка...' : 'Готовим оплату...';
            }
            ...
            try {
                ...
                // Платный тариф → уходим на страницу оплаты ЮKassa.
                // Кнопку намеренно не разблокируем: переход уже начался.
                if (outcome && outcome.redirect) {
                    window.location.href = outcome.redirect;
                    return;
                }

                // Тест-драйв — показываем успех.
                // Доступ уже открыт сервером: не обещаем письмо, которого не
                // будет, а сразу ведём ко входу. Раньше здесь стояло «следите
                // за почтой» — а письма никто не слал, и женщина просто ждала.
                const opened = !!(outcome && outcome.opened);
                if (successEmail) successEmail.textContent = email;
                if (successTitle && isTrial && opened) successTitle.textContent = 'Доступ открыт!';
                if (successText) {
                    successText.innerHTML = isTrial && opened
                        ? 'Тест-драйв открыт для <strong id="success-client-email"></strong> — дни 1–3 уже ждут. ' +
                          'Зайдите на <a href="https://lk.katyafit.ru" target="_blank" rel="noopener">lk.katyafit.ru</a>, ' +
                          'введите эту почту — придёт код для входа. Пароль не нужен.'
                        : isTrial
                ...
                if (successCloseBtn && isTrial && opened) {
                    successCloseBtn.textContent = 'Войти в кабинет';
                    successCloseBtn.dataset.goCabinet = '1';
                }
                if (successOverlay) {
                ...
                if (typeof showToast === 'function') {
                    showToast(err && err.message
                        ? err.message + ' Попробуйте ещё раз или напишите напрямую.'
                        : 'Ошибка отправки. Попробуйте снова или напишите напрямую.');
                }
            }
        });
    }
    ...
    if (successCloseBtn) {
        successCloseBtn.addEventListener('click', () => {
            ...
            // Если тест-драйв уже открыт, кнопка называется «Войти в кабинет» —
            // вести с неё на главную лендинга было бы обманом надписи.
            window.location.href = successCloseBtn.dataset.goCabinet === '1'
                ? 'https://lk.katyafit.ru'
                : 'index.html';
        });
    }
